 ...
 * 
 * to read json file kept externally :- convert the content of the file into string format (GOAL)
...
public class StaticJson {
    ...
    public void readStaticJsonFile() throws IOException {
        ...
        String response = given().log().all().queryParam("key", "qaclick123").header("Content-Type", "application/json")
                                 .body(new String(Files.readAllBytes(Paths.get("./resources/Static_AddPlace.json")))).
                                 when().post("maps/api/place/add/json").
                                 then().log().all().assertThat().statusCode(200).extract().response().asString();

        ReusableMethods rm= new ReusableMethods();
        JsonPath js=  rm.rawToJson(response);
        String placeID = js.getString("place_id"); //or js.get("place_id");
